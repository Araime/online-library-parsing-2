...
Для запуска приложение принимает два необязательных аргумента:
  ...
  1. `--end_page` или `-e` - по какую страницу скачивать книги
По умолчанию эти два параметра `-s` = 1, `-e` = 4.
...
обложки и подробное описание к каждой книге. Пример запуска:

```sh
python main.py -s 1 -s 5
```
